...
public class JWKUtil {
  ...
  public static JWKSet getJWK() throws Exception {
    ...
    requestOptions.put(
        ...
        "Bearer "
            + getBearer(
                ResourceServerConfig.CLIENT_ID,
                ResourceServerConfig.CLIENT_SECRET,
   ...
   * @throws Exception
   */
  public static String getBearer(String clientId, String clientSecret, String scope)
      throws Exception {
    String bearer = "";
    ...
    requestOptions.put(
        "Authorization",
        "Basic " + Base64.getEncoder().encodeToString(authzHdrVal.getBytes("UTF-8")));
    ...
    try {
      ...
      if (header != null) {
        for (String key : header.keySet()) {
          ...
          Logger.getLogger(JWKUtil.class.getName())
              .log(Level.INFO, "Header: " + key + ": " + header.get(key));
        }
      }
      ...
      Logger.getLogger(AccessTokenValidator.class.getName())
          .log(Level.SEVERE, "HTTP UTIL: exception :" + e.getMessage(), e);
